...
* states required.
*
* The output is in the form of N, followed by lists of integers
* separated by newlines. The blocks are to be
* thought of as numbered from 0 to N-1, with the table as number -1. Each
* state is specified by giving, for each block in sequence,
* the number of the block it is on.
* For example, the state of 6 blocks
*
*     1
*     4    2
*     3    0    5
...
*    3
*    -1
* meaning there are 6 blocks; 0 is on the table, 1 on 4, 2 on 0, 3 on the
* table, 4 on 3, and 5 on the table.
*
* Parameters are given to this program on the command line. The options are:
* 
*    -s <integer>     number of states required (default 1)
*    -r <integer>     seed for the random number generator (default PID^2)
*    <integer>        number of blocks in each state (required)
*
